...
excerpt: "Most MCP servers in production today depend on three things the spec is in the process of removing: protocol sessions, the initialize handshake, and server-initiated sampling. The replacement — stateless requests plus durable tasks — is a better fit for how agents actually run. This is the migration nobody has written up yet."
---
